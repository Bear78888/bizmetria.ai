...
describe('canonical site origin', () => {
  it('is the production domain, without a trailing slash or path', () => {
    expect(CANONICAL_SITE_URL).toBe('https://bizmetria.ai');
  });

  it('builds absolute URLs on the canonical origin', () => {
    expect(canonicalPath('/en')).toBe('https://bizmetria.ai/en');
    expect(canonicalPath('/sitemap.xml')).toBe('https://bizmetria.ai/sitemap.xml');
  });
